...
class FaceRecognitionViewState extends State<FaceRecognitionView> {
  ...
  Future<bool> onFaceDetected(faces) async {
    ...
    if (faces.length > 0) {
      ...
      log("_identifyThreshold2: $_identifyThreshold");
      log("_livenessThreshold2: $_livenessThreshold");
      if (maxSimilarity > _identifyThreshold &&
          maxLiveness > _livenessThreshold) {
        recognized = true;
        log("maxLiveness2: $maxLiveness");
        log("maxSimilarity2: $maxSimilarity");
        log("recognized: $recognized");
        log("_identifyThreshold: $_identifyThreshold");
        log("_livenessThreshold: $_livenessThreshold");
      }
    }

    log("maxLiveness1: $maxLiveness");
    log("maxSimilarity1: $maxSimilarity");
